...
const starts = async (sesName) => {
    try {
        ...
        client.on('CB:Presence', asd => {
            ...
            Client.sendText(asd.id, `@${asd.participant.split('@')[0]} actividad detectada!, tu estado de afk ha sido eliminado`)
                }
        })
        ...
        client.on('new-msg', (message) => {
			...
			if (global.tebakgambar[from] && global.tebakgambar[from].id && global.tebakgambar[from].jawaban.toLowerCase() == body.toLowerCase()) Client.reply(from, `SÍ ADIVINA IMAGEN RESPUESTA EXITOSAMENTE POR @${sender.split("@")[0]}`, message).then(() => global.tebakgambar[from] = {}) 
            ...
            if (isGroup && dataGc[from].antitagall && !message.isAdmin && (message.mentionedJidList.length == message.groupMembers.length || message.mentionedJidList.length-1 == message.groupMembers.length)){
                ...
                client.groupRemove(from, [sender]).catch(() => Client.reply(from, `Crea un bot de administración para que puedas usar las funciones antitagall`, message))
            }
			...
			if (isGroup && !message.isAdmin && dataGc[from].antilink && /chat\.whatsapp\.com/gi.test(body)){
				...
				dtclink.forEach(async l => {
					...
					if(checks.status == 200){
						...
						client.groupRemove(from, [sender]).catch(() => Client.reply(from, `Crea un bot de administración para que puedas usar las funciones antilink`, message))
					}
				})
			}
			if (!dataUser[sender]){
            ...
            if(isGroup) {
                if(afkJs.detectingAfk(from, sender)) Client.sendText(from, `@${sender.split('@')[0]} ahora no afk!`)
                if(message.message.extendedTextMessage && message.message.extendedTextMessage.contextInfo && message.message.extendedTextMessage.contextInfo.mentionedJid) {
                    ...
                    jids.forEach(jid => {
                        ...
                        Client.reply(from, `@${jid.split('@')[0]} en afk\nReason: ${takeData.reason}\nTime: ${duration.days()} Horas ${duration.hours()} en punto ${duration.minutes()} Minutos ${duration.seconds()} Segundos`)
                    })
                }
            }
        })
